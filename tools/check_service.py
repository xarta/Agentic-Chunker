#!/usr/bin/env python3
"""
Agentic Chunker — health checker and test tool.

Checks the deployed Agentic Chunker service, reads endpoint from .env,
and runs test requests.
...
def http_post_json(url: str, data: dict, timeout: int = 120) -> Tuple[int, str]:
...
def check_health(base_url: str) -> Dict[str, Any]:
    ...
    if status == 200:
        try:
            ...
            result["vllm_connected"] = data.get("vllm_connected")
...
def test_chunking(base_url: str) -> Dict[str, Any]:
    """Send a test request to the Agentic Chunker."""
    url = f"{base_url}/chunk"
    payload = {"text": TEST_TEXT, "max_propositions": 5}
    ...
    if status == 200:
        try:
            ...
            result["status"] = "ok"
            result["total_chunks"] = stats.get("total_chunks", len(chunks))
            result["total_propositions"] = stats.get("total_propositions")
            result["total_llm_calls"] = stats.get("total_llm_calls")
            result["processing_time"] = stats.get("processing_time_seconds")
            result["model"] = stats.get("model")
            if chunks:
                first = chunks[0]
                result["first_chunk_title"] = first.get("title", "")
                result["first_chunk_summary"] = first.get("summary", "")[:120]
        except json.JSONDecodeError:
            result["status"] = "error"
            result["error"] = "Invalid JSON response"
    else:
...
def format_test(result: Dict[str, Any], as_json: bool = False) -> str:
    ...
    lines = [f"  {icon:8s} Agentic Chunker test ({elapsed:.1f}s)"]

    if status == "ok":
        if "total_chunks" in result:
            lines.append(f"           chunks: {result['total_chunks']}")
        if result.get("total_propositions"):
            lines.append(f"           propositions: {result['total_propositions']}")
        if result.get("total_llm_calls"):
            lines.append(f"           llm_calls: {result['total_llm_calls']}")
        if result.get("model"):
            lines.append(f"           model: {result['model']}")
        if result.get("first_chunk_title"):
            lines.append(f"           title: {result['first_chunk_title']}")
    else:
        if result.get("error"):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check Agentic Chunker service health and run test requests.",
    )
    parser.add_argument(
    ...
    base_url = env.get("AGENTIC_CHUNKER_URL", "")

    if not base_url:
        print("Error: AGENTIC_CHUNKER_URL not set in .env", file=sys.stderr)
    ...
    if do_test:
        outputs.append("")
        outputs.append("Test: Agentic Chunker")
        outputs.append("-" * 40)
